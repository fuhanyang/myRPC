...
func (client *Client) GetHeader() header.Header {
...
func NewClient(conn net.Conn, opt *server.Option) (*Client, error) {
	...
	if f == nil {
		...
		log.Println("server client: codec error:", err)
		return nil, err
	}
	// send options with server
	if err := json.NewEncoder(conn).Encode(opt); err != nil {
		log.Println("server client: options error: ", err)
		_ = conn.Close()
		return nil, err
	}
...
func (client *Client) Go(serviceMethod string, args, reply interface{}, done chan *Call) *Call {
	if done == nil {
		...
		log.Panic("server client: done channel is unbuffered")
	}
	call := &Call{
